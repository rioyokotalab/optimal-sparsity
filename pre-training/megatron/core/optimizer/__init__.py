...
def _get_component_lr_scale(name: str, config: OptimizerConfig) -> float:
    """Get learning rate scale for a parameter based on its name and config.
    ...
    if 'decoder.final_layernorm.weight' in name:
        return config.lr_scale_final_layernorm

    # Expert layers
    elif 'mlp.experts.linear_fc' in name:
        return config.lr_scale_expert

    # Shared expert layers
    elif 'mlp.shared_experts.linear_fc' in name:
        return config.lr_scale_shared_expert

    # Gate/Router
    elif 'mlp.router.weight' in name:
        return config.lr_scale_gate

    # Pre layer norm
    elif 'pre_mlp_layernorm.weight' in name:
        return config.lr_scale_pre_layernorm

    # Attention projection
    elif 'self_attention.linear_proj.weight' in name:
        return config.lr_scale_attention_proj

    # QKV layer norm
    elif 'self_attention.linear_qkv.layer_norm_weight' in name:
        return config.lr_scale_qkv_layernorm

    # QKV transformation
    elif 'self_attention.linear_qkv.weight' in name:
        return config.lr_scale_qkv

    # Embedding
    elif 'embedding.word_embeddings.weight' in name:
        return config.lr_scale_embedding

    # Output final layer
    elif 'output_layer.weight' in name:
        return config.lr_scale_final_linear
...
def _get_param_groups(
    ...
    for model_chunk in model_chunks:
        for name, param in model_chunk.named_parameters():
            ...
            # Apply component-wise scaling if config is provided
            if config is not None:
                _lr_mult *= _get_component_lr_scale(name, config)
